...
struct FullJetSpectrapp {
  ...
  void labelCollisionHistograms(HistogramRegistry& registry)
  {
    ...
    if (doprocessTracksWeighted) {
      ...
      h_collisions_weighted->GetXaxis()->SetBinLabel(0, "total events");
      h_collisions_weighted->GetXaxis()->SetBinLabel(1, "JetsMCDWeighted with kTVXinEMC");
      h_collisions_weighted->GetXaxis()->SetBinLabel(2, "JetsMCPMCDMatchedWeighted with kTVXinEMC");
      h_collisions_weighted->GetXaxis()->SetBinLabel(3, "TracksWeighted with kTVXinEMC");
      h_collisions_weighted->GetXaxis()->SetBinLabel(4, "JetsMCDWeighted w/o kTVXinEMC");
      h_collisions_weighted->GetXaxis()->SetBinLabel(5, "JetsMCPMCDMatchedWeighted w/o kTVXinEMC");
      h_collisions_weighted->GetXaxis()->SetBinLabel(6, "TracksWeighted w/o kTVXinEMC");
      h_collisions_weighted->GetXaxis()->SetBinLabel(7, "Fake Matched Weighted MCD Jets");
      h_collisions_weighted->GetXaxis()->SetBinLabel(8, "Fake Matched Weighted MCP Jets");
    }
  }
  ...
  void init(o2::framework::InitContext&)
  {
    ...
    if (doprocessJetsData || doprocessJetsMCD || doprocessJetsMCDWeighted) {
      registry.add("h_Detcollision_counter", "event status;event status;entries", {HistType::kTH1F, {{100, 0.0, 100.}}});
    ...
    if (doprocessJetsMCP || doprocessJetsMCPWeighted) {
      registry.add("h_Partcollision_counter", "event status;event status;entries", {HistType::kTH1F, {{100, 0.0, 100.0}}});
    ...
    if (doprocessJetsMCPMCDMatched || doprocessJetsMCPMCDMatchedWeighted) {
      registry.add("h_Matchedcollision_counter", "event status;event status;entries", {HistType::kTH1F, {{100, 0.0, 100.0}}});
    ...
    if (doEMCALEventWorkaround) {
      if (collision.isEmcalReadout() && !collision.isAmbiguous()) { // i.e. EMCAL has a cell content
        ...
        if (collision.alias_bit(kTVXinEMC)) {
          ...
          registry.fill(HIST("h_Detcollision_counter"), collision.mcCollision().weight());
        }
      }
    } else {
      if (!collision.isAmbiguous() && jetderiveddatautilities::eventEMCAL(collision) && collision.alias_bit(kTVXinEMC)) {
  ...
  void processJetsMCP(typename JetTableMCPJoined::iterator const& jet, aod::JetParticles const&, aod::JetMcCollisions const&, soa::Filtered<soa::Join<EMCCollisions, aod::JMcCollisionLbs, aod::JetCollisionsMCD>> const& collisions)
  {

    for (auto const& singlemcpcollision : collisions) {

      // std::cout << "singlemcpcollision TYPE = " << typeid(singlemcpcollision).name() << std::endl;

      if (!jetderiveddatautilities::eventEMCAL(singlemcpcollision) && !singlemcpcollision.alias_bit(kTVXinEMC)) {
        break;
      }
      // if (!jetderiveddatautilities::setEventSelectionBit(singlemcpcollision) && !singlemcpcollision.selection_bit(o2::aod::evsel::kNoSameBunchPileup))  {
      //   break;
      // }
      // if (!(jetderiveddatautilities::setEventSelectionBit(singlemcpcollision) & (1 << JCollisionSel::selNoSameBunchPileup))) {
      //   continue; // Skip events that do not pass the selection
      // }
    }
    ...
    if (checkMcCollisionIsMatched) {  //basically checks if the same collisions are generated at the Part level as those at the Det level
      ...
      // if (!jetderiveddatautilities::eventEMCAL(collisionspermcpjet)) {
      //   return;
      // }
      // auto collisionmcd = collisionspermcpjet.iteratorAt();
      // if (!(collisionspermcpjet.iteratorAt(0).isEmcalReadout()))  {
      //   return;
      // }

      if (collisionspermcpjet.size() >= 1 && jetderiveddatautilities::selectCollision(collisionspermcpjet.begin(), eventSelectionBits)) {
        //Now here for every matched collision, I fill in my event counter histograms and the corresponding jet histograms.
        registry.fill(HIST("h_Partcollision_counter"), 1.0);
  ...
  void processJetsMCPWeighted(typename JetTableMCPWeightedJoined::iterator const& jet, aod::JetParticles const&, aod::JetMcCollisions const&, soa::Filtered<soa::Join<EMCCollisions, aod::JMcCollisionLbs, aod::JetCollisionsMCD>> const& collisions)
  {

    for (auto const& singlemcpcollision : collisions) {

      // std::cout << "singlemcpcollision TYPE = " << typeid(singlemcpcollision).name() << std::endl;
      // auto collision = singlemcpcollision.mcCollision();  // Extract the collision table
      // std::cout << "truecollision TYPE = " << typeid(truecollision).name() << std::endl;
      // if (!jetderiveddatautilities::selectCollision(collision, eventSelectionBits)) {
      //    return;
      // }
      if (!jetderiveddatautilities::eventEMCAL(singlemcpcollision) && !singlemcpcollision.alias_bit(kTVXinEMC)) {
      ...
      // if (!(jetderiveddatautilities::setEventSelectionBit(singlemcpcollision) & (1 << jetderiveddatautilities::JCollisionSel::selNoSameBunchPileup))) {
      //   continue; // Skip events that do not pass the selection
      // }
      // auto collision = singlemcpcollision.mcCollision();  // Extract the collision table
      // std::cout << "singlemcpcollision TYPE = " << typeid(collision).name() << std::endl;
      // uint16_t eventSel = jetderiveddatautilities::setEventSelectionBit(singlemcpcollision);
      // if (!(eventSel & (1 << jetderiveddatautilities::JCollisionSel::selNoSameBunchPileup))) {
      //   continue;
      // }
    }
    if (!jetfindingutilities::isInEtaAcceptance(jet, jetEtaMin, jetEtaMax, trackEtaMin, trackEtaMax)) {
    ...
    if (checkMcCollisionIsMatched) {
      ...
      if (collisionspermcpjet.size() >= 1 && jetderiveddatautilities::selectCollision(collisionspermcpjet.begin(), eventSelectionBits)) {
        registry.fill(HIST("h_Partcollision_counter"), jet.eventWeight());
    ...
    if (doEMCALEventWorkaround) {
      if (collision.isEmcalReadout() && !collision.isAmbiguous()) { // i.e. EMCAL has a cell content
        ...
        if (collision.alias_bit(kTVXinEMC)) {
          ...
          registry.fill(HIST("h_Matchedcollision_counter"), 1.0);
        }
      }
    } else {
      if (!collision.isAmbiguous() && jetderiveddatautilities::eventEMCAL(collision) && collision.alias_bit(kTVXinEMC)) {
  ...
  void processJetsMCPMCDMatchedWeighted(soa::Filtered<soa::Join<EMCCollisions, aod::JMcCollisionLbs, aod::JetCollisionsMCD>>::iterator const& collision, JetTableMCDMatchedWeightedJoined const& mcdjets, JetTableMCPMatchedWeightedJoined const& mcpjets, aod::JMcCollisions const&, aod::JetTracks const&, aod::JetClusters const&, aod::JetParticles const&)
  {
